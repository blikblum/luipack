...
type
  ...
  TATSearchOptions = set of TATSearchOption;

  TATSearchCallback = function (APercent: word): boolean;
...
type
  ...
  private
    ...
    FSearchCallback: TATSearchCallback;
    ...
    procedure SetSearchIndentHorz(AIndent: word);
    ...
    procedure SetPosPercent(APos: word);
    ...
    procedure SetTabSize(ASize: word);
  ...
  public
    ...
    function FindText(const AText: WideString; const AStartPos: Int64; AOptions: TATSearchOptions): Int64;
  ...
  published
    ...
    property TextSearchIndentHorz: word read FSearchIndentHorz write SetSearchIndentHorz default 5;
    property TextTabSize: word read FTabSize write SetTabSize default 8;
    ...
    //property ScrollHorzAlwaysVisible: boolean read FScrollHorzVisible write FScrollHorzVisible default false;
    //property ScrollVertAlwaysVisible: boolean read FScrollVertVisible write FScrollVertVisible default false;
...
const
  cMaxLengthDefault = 300; //Default "maximal line length".
                           //Don't set too large value, it affects default file buffer size.

  cMaxLength = 2*1024; //Limits for "maximal line length".
  cMinLength = 2;
...
const
  ...
  cMinTabSize = 1;
  cSpecialChar = '.'; //Char that replaces special chars < $20
  ...
  cPositionY0 = 0;
  cMaxClipboardDataSize = 32*1024*1024; //Max data size (DWORD) for copying to Clipboard
...
function SConvertForOut(const S: WideString; TabSize: word; AnsiDecode: boolean): WideString;
begin
...
procedure StringOut(Canvas: TCanvas; const X, Y: integer; const Str: WideString; TabSize: word; AnsiDecode: boolean);
var
  S: WideString;
begin
...
function StringExtent(Canvas: TCanvas; const Str: WideString; var Ext: TStringExtent; TabSize: word; AnsiDecode: boolean): boolean;
var
  S: WideString;
  Size: TSize;
  i, j: integer;
...
function StringWidth(Canvas: TCanvas; const Str: WideString; TabSize: word; AnsiDecode: boolean): integer;
var
  S: WideString;
  Size: TSize;
  OK: boolean;
...
begin
  if GetTextMetrics(Canvas.Handle, Metric)
    ...
    else Result:= Abs(Canvas.Font.Height);
end;
...
type
  ...
  public
    ...
    function GetPosFromCoord(ACanvas: TCanvas; AX, AY: integer; ATabSize: word; AAnsiDecode: boolean): Int64;
    function GetCoordFromPos(ACanvas: TCanvas; const APos: Int64; ATabSize: word; AAnsiDecode: boolean; var AX, AY: integer): boolean;
    function GetScreenWidth(ACanvas: TCanvas; ATabSize: word; AAnsiDecode: boolean): integer;
  end;
...
function TStrPositions.GetPosFromCoord(ACanvas: TCanvas; AX, AY: integer; ATabSize: word; AAnsiDecode: boolean): Int64;
var
  YH: integer;
  Num, i: word;
  Dx: TStringExtent;
...
function TStrPositions.GetCoordFromPos(ACanvas: TCanvas; const APos: Int64; ATabSize: word; AAnsiDecode: boolean; var AX, AY: integer): boolean;
var
  i: word;
  Dx: TStringExtent;
  //S: string;
...
function TStrPositions.GetScreenWidth(ACanvas: TCanvas; ATabSize: word; AAnsiDecode: boolean): integer;
var
  i: word;
  W: integer;
begin
  ...
  for i:= 1 to FNum do
    with FArray[i] do
      begin
      W:= Pnt.X+StringWidth(ACanvas, Str, ATabSize, AAnsiDecode);
      ILimitMin(Result, W);
      end;
end;
...
var
  ...
  begin
    if ASelectAll
      then Len:= 1
      else Len:= Length(ALine);
    if (FSelStart>AFilePos+(Len-1)*CharSize) or (FSelStart+FSelLength-1*CharSize<AFilePos) then Exit;
    ...
    if StringExtent(FBitmap.Canvas, ALine, Dx, FTabSize, GetAnsiDecode) then
      ...
      if ASelectAll then
        ...
        if nEnd>Length(ALine) then nEnd:= Length(ALine);
...
begin
  ...
  Assert(
    ...
    Format('Positions out of range: Redraw'#13#13'BufferPos: %d, ViewPos: %d', [FBufferPos, FViewPos]));
    ...
    case FMode of
        ...
        for i:= 1 to IMin(LinesNum+1, cMaxLines) do
          ...
          if i<=LinesNum then
            ...
            if FScrollPageSize then
              if Pos>=0 
                then PageSize:= Pos-FViewPos
                else PageSize:= FFileSize-FViewPos;
      ...
      vbmodeHex:
        ...
        for i:= 1 to IMin(LinesNum+1, cMaxLines) do
          ...
          Canvas.FillRect(Rect(X-FHViewPos, Y,
                               X-FHViewPos+CellWidth*(FTextWidthHex*3+2), Y2+(cHexLinesWidth div 2)));
          ...
          for j:= 0 to FTextWidthHex-1 do
            if FBufferPos+Pos+j<FFileSize then
              begin
              ch:= FBuffer[Pos+j];
              if Ord(ch)<$20 then ch:= cSpecialChar;
              LineA:= LineA+ch;
              end;
      ...
      if X<cPositionX0 then X:= cPositionX0;
      if Y<cPositionY0 then Y:= cPositionY0;
...
begin
  ...
  si.nPage:= 0;
  si.nPos:= 0;
  SetScrollInfo(Handle, ATypes[AHorz], si, true);
end;
...
begin
  ...
  si.cbSize:= SizeOf(si);
  si.fMask:= SIF_ALL;
  {if FScrollVertVisible
    then si.fMask:= si.fMask or SIF_DISABLENOSCROLL;}
  si.nMin:= 0;
  si.nMax:= Max;
  si.nPage:= Page;
  si.nPos:= Pos;
...
begin
  if                 //Hide horizontal scrollbar when:
    ...
    (FFileSize=0) or // - file is empty
    ((FMode in [vbmodeText, vbmodeUnicode]) and FTextWrap) or
                     // - in Text/Unicode modes when TextWrap is on
    ((FMode=vbmodeBinary) and FTextWidthFit and FontMonospaced(FBitmap.Canvas))
                     // - in Binary mode when TextWidthFit is on and font is monospaced
  then
    begin
    Max:= 0;
    Page:= 0;
...
begin
  if SourceAssigned then
    ...
    if not FFileOK then MsgReadError;
    end;
end;
...
begin
  case FMode of
    ...
    vbmodeHex:
      ...
      Result:= 10*CharSize; //Stub: ~middle line length
  end;
end;
...
begin
  Result:= not (
    (APos>=0) and (APos<=PosLast) and
    (APos-FBufferPos>=0) and (APos-FBufferPos<FBufferAllocSize)
    );
end;
...
//Last position at the end of file
...
begin
  if (APos<>FViewPos) and (APos>=0) then
    ...
    Cols:= ColsNum;
    FViewPos:= APos;
    if FViewPos>PosLast then FViewPos:= PosLast;
...
procedure TATBinHex.SetPosPercent(APos: word);
begin
...
begin
  if APos<=0 then PosBegin(ARedraw) else
    if APos>=FFileSize then PosEnd(ARedraw) else
      case FMode of
        vbmodeText,
        vbmodeUnicode:
  ...
  if FAutoReload then
    ...
    if not ANewFile then
      ...
      if FAutoReloadFollowTail and OldAtEnd then
        begin
        PosEnd(false);
        //Sometimes PosEnd goes not to the end of file - need to fix it.
        //Workaround:
        if not FViewAtEnd then PosEnd(false);
        end
      else
        SetPosOffset(OldViewPos, false);
      end;
...
begin
  FSize:= FFileSize;
  if FSize>1.0
    then FHexOffsetLen:= Trunc(Ln(FSize)/Ln(16))+1
    else FHexOffsetLen:= 0;
  if FHexOffsetLen<8 then FHexOffsetLen:= 8;
  if (FHexOffsetLen mod 2)>0 then Inc(FHexOffsetLen);
end;

procedure TATBinHex.SetMode(AMode: TATBinHexMode);
begin
  //No check for FMode<>AMode so file can be reloaded by setting the same mode
  FMode:= AMode;
...
begin
  ...
  FTextWidth:= AWidth;
  WLimitMin(FTextWidth, cMinLength);
  WLimitMax(FTextWidth, FMaxLengths[vbmodeBinary]);
end;
...
begin
  ...
  WLimitMin(FTextWidthHex, cMinLength);
  WLimitMax(FTextWidthHex, FMaxLengths[vbmodeHex]);
...
begin
  if AIndent<>FSearchIndent then
    ...
    WLimitMax(FSearchIndent, cMaxLines-1);
    end;
end;

procedure TATBinHex.SetSearchIndentHorz(AIndent: word);
begin
...
function BoolToSign(flag: boolean): integer;
begin
  if flag then Result:= 1 else Result:= -1;
end;

function TATBinHex.FindText(const AText: WideString; const AStartPos: Int64; AOptions: TATSearchOptions): Int64;
const
  BlockSize = 64*1024;
var
  ...
  SBufferW: WideString;
  n: integer;
  AForward, AWholeWords, ACaseSens: boolean;
begin
  Result:= -1;
  if FFileSize<CharSize then Exit;

  try
    Lock;

    AForward:= not (vbSearchBackward in AOptions);
    AWholeWords:= vbSearchWholeWords in AOptions;
    ACaseSens:= vbSearchCaseSens in AOptions;
    
    BufPosMax:= PosLast;
    NormalizePos(BufPosMax);
    ...
    if Assigned(FSearchCallback) then
      if not FSearchCallback(BufPos*100 div FFileSize) then Exit;
    ...
    repeat
      ...
      if not AForward then
        begin
        Dec(ReadPos, BlockSize-CharSize);
        if ReadPos<0 then ReadPos:= 0;
        ReadSize:= BufPos-ReadPos+CharSize;
        if ReadSize>BlockSize then ReadSize:= BlockSize;
        end;

      if not ReadSource(ReadPos, @Buffer, ReadSize, BytesRead) then
        begin
        MsgReadError;
        Exit
        end;
      ...
      if FMode=vbmodeUnicode then
        begin
        SBufferW:= SetStringW(Buffer, BytesRead, FFileUnicodeFmt=vbUnicodeFmtBE);
        n:= SFindTextW(AText, SBufferW, AForward, AWholeWords, ACaseSens, BytesRead<BlockSize);
        end
      else
        begin
        SetString(SBufferA, Buffer, BytesRead);
        if FEncoding=vencOEM then
          SBufferA:= ToANSI(SBufferA);
        n:= SFindText(AText, SBufferA, AForward, AWholeWords, ACaseSens, BytesRead<BlockSize);
        end;


      if n>0 then
        begin
        Result:= ReadPos+(n-1)*CharSize;
        Exit
        end;
      ...
      Dec(BufPos, Int64(Length(AText)+1) * CharSize * BoolToSign(AForward));
      NormalizePos(BufPos);
  ...
  try
    case FMode of
      vbmodeUnicode:
        ...
        Buffer:= GetSelText;
        if AsHex
          then Buffer:= SToHex(Buffer)
          else SReplaceZeros(Buffer);
        SCopyToClipboard(Buffer, FEncoding=vencOEM);
        end;
    end;
  except
...
begin
  ...
  if FSelLength>0 then
    begin
    if FSelLength>cMaxClipboardDataSize
      then BlockSize:= cMaxClipboardDataSize
...
var
  ...
  SelChanged: boolean;
begin
  if (AStart>=0) and (AStart<FFileSize) and (ALength>=0) then
    begin
    ...
    if SelChanged then
      ...
      SelLengthMax:= PosLast-FSelStart+CharSize;
      if FSelLength>SelLengthMax then FSelLength:= SelLengthMax;

      UpdateMenu;
      if not AScroll then Redraw;
      end;

    if AScroll then
      Scroll(AStart, FSearchIndent, FSearchIndentHorz);
...
procedure TATBinHex.Scroll(const APos: Int64; AIndentVert, AIndentHorz: integer);
const
  cRightIndent = 4;
var
  Pos: Int64;
...
begin
  ...
  //Scroll horizontally (and redraw)
  if TStrPositions(FStrings).GetCoordFromPos(
    FBitmap.Canvas, APos, FTabSize, GetAnsiDecode, PosX, PosY) then
    begin
    if not ((PosX>=0) and (PosX<ClientWidth-cRightIndent)) then
      HPosAt(PosX+FHViewPos-AIndentHorz*FontWidthDigits(FBitmap.Canvas));
    end;
end;
...
begin
  Result:= TStrPositions(FStrings).GetPosFromCoord(FBitmap.Canvas, AX, AY, FTabSize, GetAnsiDecode);
end;
...
begin
  ...
  if Button=mbLeft then
    ...
    if cSelectionByShiftClick and (Shift=[ssShift, ssLeft]) then
      begin
      SetSelection(
...
begin
  FMenuItemCopy.Enabled:= (FSelLength>0);
  FMenuItemCopyHex.Enabled:= FMenuItemCopy.Enabled and (FMode=vbmodeHex);
  FMenuItemSelectLine.Enabled:= FMode in [vbmodeText, vbmodeUnicode];
  FMenuItemSelectAll.Enabled:= not ((FSelStart=0) and (FSelLength>=NormalizedPos(FFileSize)));
...
var
  ...
  BytesRead: DWORD;
  Signature: Word absolute Buffer;
begin
  if FFileUnicodeFmt=vbUnicodeFmtUnknown then
    ...
    if SourceAssigned and (FFileSize>=2) then
      begin
      if ReadSource(0, @Buffer, BufSize, BytesRead) and
        (BytesRead>=2) and (Signature=$FFFE) then
...
begin
  Result:= TStrPositions(FStrings).GetScreenWidth(FBitmap.Canvas, FTabSize, GetAnsiDecode) + FHViewPos;
end;
...
//AStartPos may be equal to FFileSize (without -1)!
...
begin
  ...
  if ADir=vdirUp then
    begin
    if Pos>PosLast then Pos:= PosLast;
...
begin
  ...
  if ADir=vdirDown
    then Inc(NewPos, ALines*FMaxLength*CharSize)
    else Dec(NewPos, ALines*FMaxLength*CharSize);

  if NewPos<0 then NewPos:= 0;
  if NewPos>FFileSize then NewPos:= FFileSize;
  if NewPos=AStartPos then Exit;
  ...
  if NewPos<0 then
    if ADir=vdirDown
      then
        begin
        NewPos:= FindLinePos(FFileSize, vdirUp, Line); //d
        if NewPos<0 then NewPos:= 0;
        end
      else
        NewPos:= 0;
...
{
//Outdated: replaced with FViewAtEnd field
function TATBinHex.AtTheEnd: boolean;
var
  Pos: Int64;
  i: integer;
  s: WideString;
begin
  Result:= false;
  if FBufferPos+FBufferAllocSize<FFileSize then Exit;
  ...
  case FMode of
    ...
    vbmodeUnicode:
      ...
      for i:= 1 to LinesNum do
        Pos:= FindLinePos(Pos, vdirUp, s);
      if Pos<0 then Pos:= 0;
      Result:= FViewPos>=Pos;
      end;
    else
      begin
      Pos:= FFileSize-1-LinesNum*ColsNum;
      if Pos<0 then Pos:= 0;
      Result:= FViewPos>=Pos;
      end;
  end;
...
begin
  if ADir=vdirDown
    then Inc(APos, CharSize)
    else Dec(APos, CharSize);
end;

procedure TATBinHex.NextPos(var APos: Int64; ADir: TATDirection; AChars: integer);
begin
  if ADir=vdirDown
    then Inc(APos, AChars*CharSize)
    else Dec(APos, AChars*CharSize);
end;
...
begin
  ...
  if (Pos>=0) and (Pos<=PosLast) then
    ...
    if FMode=vbmodeUnicode then
      begin
      if FFileUnicodeFmt=vbUnicodeFmtBE
        then Result:= WideChar(Ord(FBuffer[Pos+1])+(Ord(FBuffer[Pos]) shl 8))
        else Result:= WideChar(Ord(FBuffer[Pos])+(Ord(FBuffer[Pos+1]) shl 8));
      end
    else
...
begin
  if (FMode<>vbmodeUnicode) and (FEncoding=vencOEM)
    then Result:= FFontOEM
    else Result:= Font;
end;
...
procedure TATBinHex.SetTabSize(ASize: word);
begin
  //No check for FTabSize<>ASize so method can be called to correct current value
  FTabSize:= ASize;
  WLimitMin(FTabSize, cMinTabSize);
  WLimitMax(FTabSize, cMaxTabSize);
end;
